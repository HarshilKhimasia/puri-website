...
if ($_POST) {
	...
	if (empty(trim($lname))) {
		...
		$validation .= "Please last enter name </br>";

	} else if (!preg_match("/^[a-zA-Z ]*$/", $lname)) {

		$validation .= "Only letters and white space allowed</br>";

	} else if ($lname === "test" || $lname === "demo" || $lname === "user") {
	...
	$sql = $conn->prepare("INSERT INTO purileads (name, lname, email, mobile, message, source, ip, device, os, browser, utm_source, utm_medium, utm_campaign, utm_content, utm_term)

          values(:name, :lname, :email, :mobile, :message, :source, :ip, :device, :os, :browser, :utm_source, :utm_medium, :utm_campaign, :utm_content, :utm_term)");
	...
	if ($sql->execute()) {
		...
		$messageBody .= "<strong>Mobile No. </strong><strong> : </strong>" . $message . "<br>";
